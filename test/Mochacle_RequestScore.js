...
const wallet_bob = new hre.ethers.Wallet(process.env.PRIVATE_KEY_BOB, provider);
...
// console.log('address_alice', address_alice)
...
async function testSubmission(server_flag, chain_flag) {
    ...
    CharlieTestOracle = new ethers.Contract(
        contract_info.address,
        contract_info.abi,
        wallet_charlie);
    
    const score_factor_pre_format = await CharlieTestOracle.SCORE_FACTOR();
...
async function solutionSubmission(test_id, server_flag, chain_flag) {
    ...
    const AliceTestOracle = new ethers.Contract(
        contract_info.address,
        contract_info.abi,
        wallet_alice);
